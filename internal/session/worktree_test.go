...
func TestWriteWorktreeLauncher_BasicContent(t *testing.T) {
	path, err := writeWorktreeLauncher("system prompt here", "user task", "/tmp/test-project", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
...
func TestWriteWorktreeLauncher_EmptyUserPrompt(t *testing.T) {
	path, err := writeWorktreeLauncher("system only", "", "/tmp/test-project", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(path)
...
func TestWriteWorktreeLauncher_SpecialChars(t *testing.T) {
	...
	path, err := writeWorktreeLauncher(system, user, "/tmp/test-project", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(path)
