...
const log = m => fs.appendFileSync('/tmp/lazyclaude-hook.log', new Date().toISOString() + ' NOTIFICATION ' + m + '\n');
...
process.stdin.on('end', () => {
  try {
    ...
    log('type=' + i.notification_type);
    if (i.notification_type !== 'permission_prompt') {
      log('SKIP not permission_prompt');
      return;
    }
